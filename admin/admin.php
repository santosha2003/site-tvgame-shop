...
switch($_POST['action']) {
  case 'add':
  $dupe = $db -> getOne("SELECT username FROM admin WHERE username='$_POST[username]'");
  if(!empty($dupe)) {
    session_start();
  ...
  if(empty($_POST[password])) {
	...
	header("Location: index.php?op=admin&username=$_POST[username]&fio=".urlencode(stripslashes($_POST[fio])));
	exit;
  }
...
switch($_GET['action']) {
  case 'del':
	...
	break;
  case 'edit':
	$res = $db -> getRow("SELECT id,username,fio,role,perm,email FROM admin WHERE id='$_GET[id]'");
	$res['action'] = 'edit';
